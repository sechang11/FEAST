...
# checkout has a static/ directory with stylesheets but no HTML. Mounting that
# gives every page a bare {"detail":"Not Found"} with nothing saying why, so
# say why instead -- the API still works, and this is a deploy step someone
# forgot rather than a broken server.
if not (STATIC / "index.html").is_file():
    @app.get("/", include_in_schema=False)
    def _no_site():
        raise HTTPException(
            503,
            "The site's pages have not been generated. Run "
            "`python web/build_site.py` (it is a build step, since "
            "web/static/*.html is gitignored). The JSON API under /api is "
            "unaffected and is working.")
